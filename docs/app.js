...
const GAME_INFO = {
  ...
  monopoly: { label: '大富翁', icon: '🎲', startText: '開始大富翁' },
...
function refreshPanels() {
  ...
  panels.monopolyGame = $('monopolyGame');
...
function initSocket() {
  ...
  if (window.bindMonopoly) window.bindMonopoly(socket, panels, showPanel);
}
...
function bindSocketEvents() {
  ...
  socket.on('room:update', (state) => {
    ...
    if (state.gameType === 'monopoly' && state.gameState === 'playing') {
      return;
    }
